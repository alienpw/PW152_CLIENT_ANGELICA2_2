...
bool globaldata_load()
{
	...
	// now load gshop3 items.
	if( !gshopFile.Open("Data\\gshop2.data", AFILE_OPENEXIST | AFILE_BINARY) )
	{
		a_LogOutput(1, "GlobalData_Load(), Failed to open gshop2 file");
		return false;
	}
	if( !gshopFile.Read(&global_gshop_timestamp3, sizeof(DWORD), &dwReadLen) )
	{
		a_LogOutput(1, "GlobalData_Load(), corrupted gshop2 file!");
		return false;
	}
	if( !gshopFile.Read(&size, sizeof(int), &dwReadLen) )
	{
	...
	for(i=0; i<size; i++)
	{
		...
		if( !gshopFile.Read(&retcode, sizeof(int), &dwReadLen) )
		{
			a_LogOutput(1, "GlobalData_Load(), gshop2 retcode error!");
			return false;
		}
...
bool globaldata_save_to(const char * szPath)
{
	...
	for(i=0; i<size; i++)
	{
		...
		fwrite(&item, 1, sizeof(item), file);
		fwrite(&retcode, 1, sizeof(int), file);
	}
	for(i=0; i<8; i++)
	{
...
bool globaldata_loadserver()
{
	...
	// gshop2
	if( !gshopFile.Open("gshopsev2.data", AFILE_OPENEXIST | AFILE_BINARY) )
	{
		a_LogOutput(1, "GlobalData_Load(), Failed to open gshopsev2.data file");
		return false;
	}
	if( !gshopFile.Read(&global_gshop_timestamp3, sizeof(DWORD), &dwReadLen) )
	{
