...
const char * DataTypeName[DT_MAX+1] =
{
	...
	/*"DT_NPC_LIB_PRODUCE_SERVICE", // Alien - Transmutacao
	"DT_LIB_PRODUCE_RECIPE", // Alien - Transmutacao*/
...
{	
	const int NUM_ADDON = ARRAY_SIZE(data.targets);
	float r[NUM_ADDON];
	int i;

	eliminate_zero_item((unsigned char*) data.targets, sizeof(data.targets[0]), ARRAY_SIZE(data.targets));
	
	for(i=0; i<NUM_ADDON; i++)	r[i] = data.targets[i].probability;
	NormalizeRandom(r, NUM_ADDON);
	for(i=0; i<NUM_ADDON; i++)	data.targets[i].probability = r[i];

	lib_produce_recipe_array.push_back(data);

	unsigned int pos = lib_produce_recipe_array.size()-1;
	add_id_index(ID_SPACE_ESSENCE, id, DT_LIB_PRODUCE_RECIPE, pos, &(lib_produce_recipe_array[0]));
}*/
...
//	talk_proc hashmap has been set up	
	...
	//ADD_HASH_MAP(essence, DT_LIB_PRODUCE_RECIPE, lib_produce_recipe_array);
