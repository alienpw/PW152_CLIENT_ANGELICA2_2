...
bool CECIvtrAstrolabe::SetItemInfo(BYTE* pInfoData, int iDataLen)
{
	try
	{
		...
		// Formatar dados em hexadecimal
		AString hexStr;
		for (int i = 0; i < iDataLen; ++i) {
			AString byteStr;
			byteStr.Format("%02X ", pInfoData[i]);
			hexStr += byteStr;
		}

		// Adicionar dados hexadecimais ao log
		str.Format("Exp: %d\nLevel: %d\nData (hex): %s", dr.Read_int(), dr.Read_int(), hexStr.GetBuffer(0));
		...
		// 加入附加属性说明
		{
			...
			int addon_count = sizeof(m_pDBEssence->rands) / sizeof(m_pDBEssence->rands[0]);
			int i, iSize = 0;
			for(i = 0;i < addon_count;i++) {
				if(m_pDBEssence->rands[i].id_rand != 0)
					iSize ++;
			}
			...
			if(iSize > 0 && m_aProps.GetSize() == 0) {
				...
				for(i = 0; i < addon_count; i++) {
					if(m_pDBEssence->rands[i].id_rand != 0) {
						PROPERTY& Prop = m_aProps[i];
						Prop.iType = m_pDBEssence->rands[i].id_rand;
						Prop.bEmbed = false;
						Prop.bSuite = false;
						Prop.bEngraved = false;
						Prop.bLocal = false;
						...
						DATA_TYPE DataType = DT_INVALID;
						const void* pData = pDataMan->get_data_ptr(m_pDBEssence->rands[i].id_rand, ID_SPACE_ADDON, DataType);
						if (DataType != DT_EQUIPMENT_ADDON)	{
							ASSERT(DataType == DT_EQUIPMENT_ADDON);
							return 0;
						}
