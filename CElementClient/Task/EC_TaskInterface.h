...
public:		//	Operations
	...
	bool HasIconStateID(unsigned long ulIconStateID);
	void OnTowerTaskDeliver(bool bSuccess);		// 爬塔任务发放
	void OnTowerTaskComplete(bool bSuccess);	// 爬塔任务完成
	void DeliverSoloTowerChallengeScore(int score);	// 发放单人爬塔积分奖励(积分上限和当前积分都要增加)
	int GetVIPLevel();
