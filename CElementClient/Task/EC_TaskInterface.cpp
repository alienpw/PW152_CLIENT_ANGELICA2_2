...
bool CECTaskInterface::HasIconStateID(unsigned long ulIconStateID)
{
	//return m_pHost->IsExistTeamVisibleState(ulIconStateID);
	return true;
}

void CECTaskInterface::OnTowerTaskDeliver(bool bSuccess)
{
	//m_pHost->PlayerSoloChallengeStartTask(bSuccess);
}

void CECTaskInterface::OnTowerTaskComplete(bool bSuccess)
{
	//m_pHost->PlayerSoloChallengeStageComplete(bSuccess);
}

void CECTaskInterface::DeliverSoloTowerChallengeScore(int score)
{
	//m_pHost->PlayerDeliverSoloChallengeScore(score);
}

int CECTaskInterface::GetVIPLevel()
{
	//return m_pHost->GetCashVipLevel();
	return 0;
}
