...
bool CECHostPlayer::ProcessMessage(const ECMSG& Msg)
{
	...
	case MSG_HST_PRAY_DISTANCE_CHANGE:
		...
		break;
	case MSG_PM_RANKINFO:
		OnMsgRankInfo(Msg);
		break;
	case MSG_HST_ASTROLABE_INFO_NOTIFY:		OnMsgMeridiansNotify(Msg);	break;
	}
	return true;
}
...
void CECHostPlayer::OnMsgHstExtProp(const ECMSG& Msg)
{
	...
	m_BasicProps.iKills = pCmd->kills;
	m_BasicProps.iDeaths = pCmd->deaths;
}
...
void CECHostPlayer::OnMsgRankInfo(const ECMSG &Msg)
{
	using namespace S2C;
	if (Msg.dwParam2 == RANK_INFO)
	{
		cmd_rank_info* pCmd = (cmd_rank_info*)Msg.dwParam1;
		SetRank(pCmd);
	}
}

void CECHostPlayer::OnMsgAstrolabeInfoNotify(const ECMSG& Msg)
{
	using namespace S2C;
	astrolabe_info_notify* pCmd = (astrolabe_info_notify*)Msg.dwParam1;
	
	AstrolabeInfo info;
	info.exp	= pCmd->exp;
	info.level	= pCmd->level;	

	AString str;
	str.Format("[Astrolabe Info]\nExp: %d\nLevel: %d", pCmd->exp, pCmd->level);
	OutputDebugStringA(str);

	SetAstrolabeInfo(info);
}