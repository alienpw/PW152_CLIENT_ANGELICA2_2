	...
	for( i = 0; i < nRows; i++ ){
		for( j = 0; j < nCols; j++ ){
			...
			if( nIndex < (int)srcImgVec.size() ){
				...
				for (int i = 0; i < 1; i++)
				{
					g_pGame->GetA3DDevice()->GetD3DDevice()->UpdateSurface(srcImgVec[nIndex]->GetDXSurface(), a_rc, pDestSurface->GetDXSurface(), a_pt);
				}
			}
		}
	}
...
bool CECRandomMapProcess::CopyRects(A3DSurface* pDestSurface, A3DSurface* pSrcSurface, const RECT* pSourceRectsArray, int nNumRects, const POINT* pDestPointsArray)
{
	if (!pDestSurface->GetDXSurface())
		return false;

	IDirect3DDevice9* pD3DDevice = g_pGame->GetA3DDevice()->GetD3DDevice();

	for (int i = 0; i < nNumRects; i++)
	{
		if (pD3DDevice->UpdateSurface(pSrcSurface->GetDXSurface(), &pSourceRectsArray[i], pDestSurface->GetDXSurface(), &pDestPointsArray[i]) != D3D_OK)
			return false;
	}
...
//	bool bval = pMan->LoadSurfaceFromFile(mapfileName, 0, &pTargetSurface);
	...
	a_pt[0].x = W_LEFT;
	a_pt[0].y = H_TOP;
	bool val = CopyRects(pDestSurface, pTargetSurface, a_rc, 1, a_pt);
	ASSERT(val);
	...
	a_pt[0].x = 0;
	a_pt[0].y = H_TOP;
	val = CopyRects(pDestSurface, pBorder[0], a_rc_left, 1, a_pt);
	ASSERT(val);
	...
	a_pt[0].x = 0;
	a_pt[0].y = 0;
	val = CopyRects(pDestSurface, pBorder[1], a_rc_top, 1, a_pt);
	ASSERT(val);

	// right border
	a_pt[0].x = W_TOP-W_LEFT;
	a_pt[0].y = H_TOP;
	val = CopyRects(pDestSurface, pBorder[2], a_rc_left, 1, a_pt);
	ASSERT(val);
	
	// bottom border
	a_pt[0].x = 0;
	a_pt[0].y = H_LEFT + H_TOP;
	CopyRects(pDestSurface, pBorder[3], a_rc_top, 1, a_pt);
