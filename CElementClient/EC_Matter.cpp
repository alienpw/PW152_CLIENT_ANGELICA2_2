...
CECMatter::CECMatter(CECMatterMan* pMatterMan)
{
	...
	m_pModel		= NULL;
	m_pGfx			= NULL;
	m_pGfxLoc		= NULL; // Alien - adicionado
	m_szModelFile	= NULL;
	m_szName		= NULL;
	m_fDistToHost	= 0.0f;
	...
	isMatterGFX 	= false; // Alien - adicionado
	m_pPateName		= NULL;
	m_dwMatterType	= 0;
	m_iLevelReq		= 0;
	m_nBrushes		= 0;
...
bool CECMatter::Init(const S2C::info_matter& Info)
{
	...
	/* Alien - adicionado */
	LoadGFXLocFromFile("alien.gfx", 20);
	m_pGfxLoc->SetVisible(false);
	isMatterGFX = false;
	/*Ate aqui*/
...
void CECMatter::SetDirAndUp(const A3DVECTOR3& vDir, const A3DVECTOR3& vUp)
{
	...
	/* Alien - adicionado */
	if (m_pGfxLoc)
		m_pGfxLoc->SetDirAndUp(vDir, vUp);
	/*Ate aqui*/
}
...
bool CECMatter::Tick(DWORD dwDeltaTime) 
{
	...
	/*Alien - adicionado abaixo*/
	if (isMatterGFX){
		m_pGfxLoc->TickAnimation(dwDeltaTime);	
		m_pGfxLoc->SetVisible(true);
	}
	else 
	{
		m_pGfxLoc->SetVisible(false);
...
bool CECMatter::InternalRender(CECViewport* pViewport, bool bHighLight)
{
	...
	/*Alien - adicionado abaixo*/
	if (m_pGfxLoc)
		g_pGame->GetGFXCaster()->GetGFXExMan()->RegisterGfx(m_pGfxLoc);
	/*Ate aqui*/
...
bool CECMatter::LoadGFXLocFromFile(const char* szFile, BYTE byScale)
{
	m_pGfxLoc = g_pGame->GetGFXCaster()->GetGFXExMan()->LoadGfx(g_pGame->GetA3DDevice(), szFile, false);
	if (!m_pGfxLoc)
		return false;

	float fScale = byScale * 16.0f / 256.0f;
	m_pGfxLoc->SetScale(fScale);
	m_pGfxLoc->Start();
