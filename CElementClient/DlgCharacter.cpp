...
void CDlgCharacter::RefreshHostDetails()
{
	...
	a_sprintf(szText, _AL("%d/%d"), rbp.iExp, GetHostPlayer()->GetLevelUpExp(rbp.iLevel));
...
*/
	...
	a_sprintf(szText, _AL("%d"), rbp.iAntiDefenseDegree);
	GetDlgItem("Txt_AntiDefenseDegree")->SetText(szText);
	strText.Format(GetStringFromTable(11525), (float)(rbp.iAntiDefenseDegree/(rbp.iAntiDefenseDegree+10000.f) * 100));
	GetDlgItem("Lbl_AntiDefenseDegree")->SetHint(strText);
	GetDlgItem("Lbl_AntiDefenseDegree")->SetText(g_pGame->GetItemDesc()->GetWideString(ITEMDESC_PENET_FISICA));

	a_sprintf(szText, _AL("%d"), rbp.iAntiResistanceDegree);
	GetDlgItem("Txt_AntiResistanceDegree")->SetText(szText);
	strText.Format(GetStringFromTable(11526), (float)(rbp.iAntiResistanceDegree/(rbp.iAntiResistanceDegree+10000.f) * 100));
	GetDlgItem("Lbl_AntiResistanceDegree")->SetHint(strText);
	GetDlgItem("Lbl_AntiResistanceDegree")->SetText(g_pGame->GetItemDesc()->GetWideString(ITEMDESC_PENET_MAGICA));

	a_sprintf(szText, _AL("%d"), rbp.iKills);
	GetDlgItem("Txt_RankKill")->SetText(szText);
	a_sprintf(szText, _AL("%d"), rbp.iDeaths);
	GetDlgItem("Txt_RankDeath")->SetText(szText);
