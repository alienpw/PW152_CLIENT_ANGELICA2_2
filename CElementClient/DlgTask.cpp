...
void CDlgTask::RefreshTaskTrace()
{
	...
	if (showType == CDlgTaskTrace::ST_TRACED || showType == CDlgTaskTrace::ST_TITLE) {
		...
		{
			// &tasks[0] was tasks.begin()....
			if ( tasks.size() > 0 )			
				pDlgTaskTrace->RefreshTaskTrace(&tasks[0], tasks.size(), titletasks.begin(), titletasks.size(), true);			
			else pDlgTaskTrace->RefreshTaskTrace(NULL, 0, titletasks.begin(), titletasks.size(), true); // Alien - Adicionado aqui para fixar o task trace para sumir ao cancelar missao
		}
