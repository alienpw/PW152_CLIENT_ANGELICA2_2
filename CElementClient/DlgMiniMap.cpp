...
#include "Alien/DlgFortunePallete.h"
...
AUI_ON_COMMAND("addmoretime",	OnCommand_addmoretime)
...
AUI_ON_EVENT("Btn_Box",		WM_LBUTTONUP,		OnEventLButtonUp_Btn_Box)
AUI_ON_EVENT("Img_Portal",	WM_LBUTTONDOWN,		OnEventLButtonDown_Teleport)
AUI_ON_EVENT("Btn_Live",	WM_LBUTTONUP,		OnEventOpenTransmutation)
...
void CDlgMiniMap::OnTick()
{
	if( stricmp(GetName(), "Win_Map") == 0 )
	{
		...
		int world_id = GetWorld()->GetInstanceID();
		PAUILABEL lblTime = (PAUILABEL)GetDlgItem("Txt_Time");
		if (world_id == 132) 
		{
			lblTime->Show(true);
			GetDlgItem("Btn_AddMoreTime")->Show(true);
			m_pBtn_Teleport->Show(true); 
		}
		else 
		{
			lblTime->Show(false);
			GetDlgItem("Btn_AddMoreTime")->Show(false);
			m_pBtn_Teleport->Show(false);
		}
...
bool CDlgMiniMap::OnInitDialog()
{
	...
	if( stricmp(GetName(), "Win_Map") == 0 )
	{
		...
		m_pBtn_Teleport =  (PAUIIMAGEPICTURE)GetDlgItem("Img_Portal");
...
void CDlgMiniMap::OnCommand_addmoretime(const char * szCommand)
{
	c2s_SendDebugCmd(50006, 1, GetHostPlayer()->GetCoolTime(GP_CT_TELEPORT_CUBO));
}
...
bool CDlgMiniMap::UpdateMiniMap()
{
	...
	nMax = pHost->GetCoolTime(GP_CT_RANDOM_TELEPORT);
	ACString str;
	AUIClockIcon* pClock = m_pBtn_Teleport->GetClockIcon();
	if( nMax > 0 )
		{
			pClock->SetProgressRange(0, 10000);
			pClock->SetProgressPos(10000 -nMax);
			pClock->SetColor(A3DCOLORRGBA(0, 0, 0, 128));
		}
	PAUILABEL lblTime = (PAUILABEL)GetDlgItem("Txt_Time");
	ACString tmpTime;
	int ctTime = pHost->GetCoolTime(GP_CT_TELEPORT_CUBO);	
	lblTime->SetText(pGameUI->GetFormatTimeMS(ctTime));
	lblTime->SetColor((ctTime / 1000) < 10 ? A3DCOLORRGB(210, 0, 0) : A3DCOLORRGB(255,255,255));
...
void CDlgMiniMap::OnEventLButtonUp_Btn_Box(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	GetGameUIMan()->CreateDlg(g_pGame->GetA3DEngine(), g_pGame->GetA3DDevice(), "continuousmain.xml");
	CDlgFortunePallete *pDlg = dynamic_cast<CDlgFortunePallete*>(GetGameUIMan()->GetDialog("Win_ContinousMain"));
	if (pDlg)
	{
...
void CDlgMiniMap::OnEventOpenTransmutation(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	
}

void CDlgMiniMap::OnEventLButtonDown_Teleport(WPARAM wParam, LPARAM lParam, AUIObject *pObj)
{
	c2s_SendDebugCmd(50002, 0);	
}
