...
int CDlgWorldMap::GetTravelWorldID()
{
	...
	else if (dwType == DT_TRANSMITSCROLL_ESSENCE && tempMapId > 0)
		worldid = tempMapId;
	...
	if( id >= 0 )
	{
		...
		else if(GetData() == DT_TRANSMITSCROLL_ESSENCE)
		{
			...
			else
			{
				...
				const CECMapDlgsMgr::TransWay &transWay = pMgr->GetTransWay();
				const CECMapDlgsMgr::TransWayPoints &transWayPoints = transWay.wayPoints;
				if (transWayPoints.GetCount()>0){
					ALISTPOSITION pos = transWayPoints.GetHeadPosition();
					while(pos)
					{ 
						CECMapDlgsMgr::TRANS_POINT* tp = transWayPoints.GetNext(pos);
						c2s_SendDebugCmd(8887, 4, tp->worldid, (int)tp->vecPos.x, (int)tp->vecPos.y, (int)tp->vecPos.z);
						OnCommand_CANCEL("IDCANCEL");						
					}
				}				
			}
...
void CDlgWorldMap::BuildTravelMap(DWORD dwType, void* pData, int mapID)
{
	// default world id
	int worldid = MAJOR_MAP;
	tempMapId = mapID;
	...
	else if(dwType == DT_TRANSMITSCROLL_ESSENCE)
	{
		...
		if (mapID == 0)
			...
			pMgr->LoadMapTexture2(m_pA3DDevice, pImage, mapID);
	}
	else
	{
		// invalid transmit type
...
AString CECMapDlgsMgr::GetMapTexture2(int idInstance){	
	AString strFile;
	CECGameRun *pGameRun = g_pGame->GetGameRun();	
	if(idInstance == MAJOR_MAP)	
		strFile = "InGame\\JourneyMap.dds";
	else{
		CECInstance *pInstance = pGameRun->GetInstance(idInstance);
		AString strSubPath = pInstance->GetPath();
...
bool CECMapDlgsMgr::LoadMapTexture2(A3DDevice* pDevice, PAUIIMAGEPICTURE pImage, int idInstance)
{
	AString strFile = GetMapTexture2(idInstance);
	A2DSprite *pA2DSprite = new A2DSprite;
